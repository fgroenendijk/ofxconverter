...
##        photo = PhotoImage(file="chameleon.gif")
    ...
    def initUI(self):
        ...
        notebook = tkinter.ttk.Notebook( self.parent )
        tabFilesMain = Frame( notebook )

        self.tabFiles = Frame( tabFilesMain )
        self.tabFiles.pack(fill=constants.BOTH)

        button = tkinter.ttk.Button( tabFilesMain, command=self.parseFile, text="Process" )
        ...
        self.currencyCombo = tkinter.ttk.Combobox( currencyLine,width=30,text="currency" )
        ...
        self.creditCombo = tkinter.ttk.Combobox(configurationFrame,width=10,text="credit")
        ...
        self.debitCombo = tkinter.ttk.Combobox( configurationFrame,width=10,text="debit" )
    ...
    def addFile(self,filename,ibans):
        if filename != "" and len(ibans) > 0:
            ...
            combo = tkinter.ttk.Combobox(self.tabFiles,values=ibanList)
            combo.current(0)
            if len(ibanList) == 1:
            ...
            self.row += 1
            tkinter.ttk.Separator(self.tabFiles).grid(row=self.row, sticky="ew", columnspan=3 )
            self.row += 1
    ...
    def addFileToCustomTab(self,filename):
        if filename != "":
            ...
            for row in file:
                ...
                for field in row:
                    if lines == 1:
                        combo = tkinter.ttk.Combobox(self.frame,values=transaction.fields,state="readonly")
                        combo.current(0)
                        combo.grid(row=0,column=column,sticky=constants.W)
                        self.comboBoxes.append( combo )
                        combo.bind('<<ComboboxSelected>>', self.onComboboxChanged)
                        nextColumn = column + 1
                        tkinter.ttk.Separator(self.frame,orient=constants.VERTICAL).grid(row=0, column=nextColumn, sticky="ns")
                    ...
                    column = column + 1
                    tkinter.ttk.Separator(self.frame,orient=constants.VERTICAL).grid(row=lines, column=column, sticky="ns")
                    column = column + 1
